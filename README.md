...
- **근무자 PIN**: 직원 관리에서 직원별 PIN을 설정하면 출퇴근·업무기록·체크리스트 작성 시 본인 확인을 요구한다 (세션당 1회). PIN이 없는 직원은 그대로 사용 가능.
- **관리자 PIN**: 설정 화면에서 등록하면 관리자 메뉴(근로기록 관리, 스케줄, 영상, 직원, 설정) 진입 시 PIN을 요구한다. 비워서 저장하면 잠금 해제.
- PIN은 bcrypt 해시로 저장되며 백엔드 밖으로 나가지 않는다. 구버전 평문 PIN은 검증 성공 시 자동으로 해시로 승격된다.
